...
require_once(O3_CMS_THEME_DIR.'/classes/snapfer_countries.php');

//Require define class
require_once(O3_CMS_THEME_DIR.'/classes/snapfer_helper.php');

/**
 * O3 Snapfer Country class
 *
 * @package o3 cms
 * @link    todo: add url
...
//Default country id
snapfer_helper::def('DEFAULT_COUNTRY',46);

class snapfer_country extends o3_cms_object {
	...
	*/
	public function load( $id ) {				
		if ( $id > 0 )
			$this->data = o3_with(new snapfer_countries())->get_by_id( $id );			
	}
	...
	*/
	public function load_by_ip( $ip ) {				
		$this->data = o3_with(new snapfer_countries())->get_by_ip( $ip );			
	}
	...
	*/
	public function format_date( $date, $show_time = false ) {
		return date( ( $this->is() ? $this->get('date_format') : 'j/n/Y' ).( $show_time ? ' H:i' : '' ), $this->o3->mysqli->date2time( $date ) );
	}
