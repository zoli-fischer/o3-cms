...
/**
 * O3 Snapfer Email sending class
 *
 * @package o3 cms
 * @link    todo: add url
...
snapfer_helper::def('SNAPFER_EMAIL_FROM', 'Snapfer <no-reply@'.$_SERVER['HTTP_HOST'].'>');

//Email queue expiration in days
snapfer_helper::def('SNAPFER_EMAIL_QUEUE_DAYS',180);

//Email queue expiration in seconds
snapfer_helper::def('SNAPFER_EMAIL_QUEUE_SEC',SNAPFER_EMAIL_QUEUE_DAYS * 24 * 3600);

//Email queue cleaning percent - default: 1
snapfer_helper::def('SNAPFER_EMAIL_QUEUE_CLEAN_USE_PERCENT',1);

class snapfer_emails extends o3_cms_objects {
	...
	* @return Boolean
	*/
	public function send( $to, $subject, $message, $priority = 0, $cc = '', $bcc = '', $from = SNAPFER_EMAIL_FROM ) {
		...
		//delete old requests
		if ( SNAPFER_EMAIL_QUEUE_CLEAN_USE_PERCENT > 0 && rand(1,100) <= SNAPFER_EMAIL_QUEUE_CLEAN_USE_PERCENT ) {
			$this->o3->debug->_('Clearing old password reset request');
			
			$datetime = date('Y-m-d H:i:s', time() - SNAPFER_EMAIL_QUEUE_SEC );
	...
	* @return Boolean
	*/
	public function sendto( $to, $subject, $message, $cc = '', $bcc = '', $from = SNAPFER_EMAIL_FROM ) {
