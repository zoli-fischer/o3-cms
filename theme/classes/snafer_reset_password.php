...
/**
 * O3 Snafer Reset password class
 *
 * @package o3 cms
 * @link    todo: add url
...
snafer_helper::def('SNAFER_PASSWORD_RESET_REQUEST_HOURS',12);

//Request expiration in seconds
snafer_helper::def('SNAFER_PASSWORD_RESET_REQUEST_SEC',SNAFER_PASSWORD_RESET_REQUEST_HOURS * 3600);

//Request cleaning percent - default: 1
snafer_helper::def('SNAFER_PASSWORD_RESET_REQUEST_CLEAN_USE_PERCENT',1);

class snafer_reset_password extends o3_cms_object {

	/*
	* Get table name
	*/
	function tablename() {
		return $this->o3->mysqli->tablename('snafer_reset_password_requests');
	}
	...
	*/
	public function release() {
		...
		//delete old requests
		if ( SNAFER_PASSWORD_RESET_REQUEST_CLEAN_USE_PERCENT > 0 && rand(1,100) <= SNAFER_PASSWORD_RESET_REQUEST_CLEAN_USE_PERCENT ) {
			$this->o3->debug->_('Clearing old password reset request');
			
			$datetime = date('Y-m-d H:i:s', time() - SNAFER_PASSWORD_RESET_REQUEST_SEC );
	...
	*/
	public function send() {
		if ( $this->is() ) {
			$user = new snafer_user( $this->get('user_id') );
			if ( $user->is() )
				return o3_with(new snafer_emails())->send( $user->get('email'), 'Reset your password', '<h1>Hello.</h1> <p>You can reset your Snafer password by clicking the link below: <a href="'.o3_html($this->url()).'" target="_blank">'.o3_html($this->url()).'</a></p> <p>Your username is: '.o3_html($user->get('username')).'</p> <p>If you didn\'t request a password reset, feel free to delete this email.</p>' );
		}
		return false;
	}
