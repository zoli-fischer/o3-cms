/*
* Snafer create transfer controller
*/
function snaferUploadApp( opts ) {
	...
	self.max_upload_size_display = ko.computed(function() {
		switch ( self.parent().logged_user.subsciption_type() ) {
			case SNAFER_FREE:
				return SNAFER_TRANSFER_FREE_MAXSIZE_GB;
			case SNAFER_PREMIUM:
				return SNAFER_TRANSFER_PREMIUM_MAXSIZE_GB;
		};
		return SNAFER_TRANSFER_MAXSIZE_GB;
	});
	...
	self.max_upload_size = ko.computed(function() {
		switch ( self.parent().logged_user.subsciption_type() ) {
			case SNAFER_FREE:
				return SNAFER_TRANSFER_FREE_MAXSIZE;
			case SNAFER_PREMIUM:
				return SNAFER_TRANSFER_PREMIUM_MAXSIZE;
		};
		return SNAFER_TRANSFER_MAXSIZE;
	});
			...
			if ( is_file ) {
				...
				if ( file.size + self.files_size() > self.max_upload_size() ) {
					...
					if ( !self.is_in_files(file) ) {
						self.files.push( new snaferUploadFileApp({
							file: file,
							parent: self
						}) );
					} else {
